...
def add_book(cursor: sqlite3.Cursor, connection: sqlite3.Connection) -> None:
    try:
...
def update_price(cursor: sqlite3.Cursor, connection: sqlite3.Connection) -> None:
    try:
...
def delete_book(cursor: sqlite3.Cursor, connection: sqlite3.Connection) -> None:
    try:
...
def search_by_author(cursor: sqlite3.Cursor) -> None:
    keyword = input("Enter a author keyword: ").strip()

    cursor.execute(
        """
        SELECT bookId, author, author, price
        FROM book
        WHERE author LIKE ?
        ORDER BY author
        """,(f"%{keyword}%",)
    ...
    print_divider()
    print("Matching books by author")
